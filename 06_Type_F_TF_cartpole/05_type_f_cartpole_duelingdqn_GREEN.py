...
class DQN:
    ...
    def __init__(self, session: tf.Session, state_size: int, action_size: int, name: str="main") -> None:
        ...
        self.H_SIZE_15_state, self.H_SIZE_15_action = 256, 256
    ...
    def build_model(self):
        with tf.variable_scope(self.net_name):
            ...
            net_1 = tf.layers.dense(net_0, self.hidden1, activation=tf.nn.relu)
            
            net_15_state = tf.layers.dense(net_1, self.H_SIZE_15_state, activation=tf.nn.relu)
            net_15_action = tf.layers.dense(net_1, self.H_SIZE_15_action, activation=tf.nn.relu)
            
            net_16_state = tf.layers.dense(net_15_state, self.action_size)
            net_16_action = tf.layers.dense(net_15_action, self.action_size)
