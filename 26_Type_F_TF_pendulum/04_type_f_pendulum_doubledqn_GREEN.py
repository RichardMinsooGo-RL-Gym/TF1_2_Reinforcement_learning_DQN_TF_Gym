...
def train_model(agent, target_agent):
    ...
    for state,action,reward,next_state, done in minibatch:
        q_update = reward
        if not done:
            tgt_q_value_next = target_agent.predict(next_state)[0]
            q_value_next = agent.predict(next_state)[0]
            q_update = reward + agent.discount_factor * tgt_q_value_next[np.argmax(q_value_next)]
...
def main():
    ...
    with tf.Session() as sess:
        ...
        pylab.savefig(graph_path + "/pendulum_doubledqn.png")
